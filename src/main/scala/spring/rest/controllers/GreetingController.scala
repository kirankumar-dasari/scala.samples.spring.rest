...
import spring.rest.model.{ErrorMessage, Greeting, Id, Message}
import spring.rest.processors.GreetingProcessor
...
class GreetingController @Autowired()(greetingProcessor: GreetingProcessor) {
...
//  }
    ...
    def hello(@RequestBody greeting: Greeting): ResponseEntity[Any] = {
      println(greeting)
      val id = greetingProcessor.process(greeting)
      new ResponseEntity(Message(id.get.toString), HttpStatus.CREATED)
    }
}
